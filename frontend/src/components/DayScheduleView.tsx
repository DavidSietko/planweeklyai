...
import { Schedule, Day, MandatoryTask, Task} from '../utils/interfaces';
import { 
    getDaySchedule, 
    DAY_LABELS, 
    formatTime, 
...
export default function DayScheduleView({ schedule, day }: DayScheduleViewProps) {
    ...
    const totalTasks = daySchedule.tasks.length + daySchedule.mandatory_tasks.length;
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
            ...
            {/* Mandatory Tasks */}
            {daySchedule.mandatory_tasks.length > 0 && (
                <div className="mb-6">
                    ...
                    <div className="space-y-3">
                        {daySchedule.mandatory_tasks.map((task) => (
...
interface TaskItemProps {
    task: Task; // Using any for now since we're importing from interfaces
    day: Day;
}

function TaskItem({ task, day }: TaskItemProps) {
    const isWeekendDay = isWeekend(day);
    const showWeekendNote = isWeekendDay && !task.on_weekends;
                ...
                <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                        ...
                        {task.preferred_time && (
                            <>
                                <span>•</span>
                                <span>Preferred: {task.preferred_time.charAt(0).toUpperCase() + task.preferred_time.slice(1)}</span>
                            </>
                        )}
                        {task.priority && (
                            <>
...
interface MandatoryTaskItemProps {
    task: MandatoryTask; // Using any for now since we're importing from interfaces
}
        ...
        <div className="p-4 border border-red-200 rounded-md bg-red-50">
            <div className="flex justify-between items-start">
                <div className="flex-1">
                    <h3 className="font-medium text-gray-900 mb-1">{task.summary}</h3>
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                        <span>Time: {formatTime(task.start_time)} - {formatTime(task.end_time)}</span>
                        {task.location && (
                            <>
                                <span>•</span>
